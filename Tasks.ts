 ...
 */
export function listTasks(taskListId: string): BucketedTasks {
...
export function markTaskComplete(taskListId: string, taskId: string) {
...
export function markTaskDelete(taskListId: string, taskId: string) {
  const task = Tasks.Tasks.get(taskListId, taskId);
  const request = {
    "deleted": true,
  };
